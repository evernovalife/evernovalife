...
**Phase 2 status (2026-09-14):** H1–H4, L1, L2, M1 (headers, no CSP), M2, M4 fixed and committed on branch `security-audit` (commits `90bf777`…`a540e70`). H6 blocked on confirming Render's Node version. H5 (move the session token off `localStorage`) and M3 (confirm `ALLOWED_ORIGINS` in the Render dashboard) deferred by request. C1's git-history exposure has no code fix — see ROTATE NOW. Per-finding status is marked inline below.
...
**H5. Session JWT and the ADMIN_KEY fallback credential both live in `localStorage`** — **DEFERRED, by request**: token versioning (M2) is fixed, which caps a stolen token's usable life at "until the account holder resets their password" instead of "until natural 30-day expiry." Actually moving the token off `localStorage` to an `httpOnly` cookie needs a dedicated pass — cross-origin cookie (`SameSite=None; Secure`) since GoDaddy and Render are different origins, `cors({ credentials: true })` instead of the current origin-echo config, and every frontend file that reads `enl_token`/`enl_admin_key` rewired. Held open on your instruction rather than rushed.
...
**M2. Password reset does not invalidate previously-issued session tokens** — **FIXED** (commit `a540e70`): a `tokenVersion` claim now rides in the JWT, bumped on reset, checked on every verify. The other half of this finding (moving the token off `localStorage` entirely, H5) stays deferred — see H5.
