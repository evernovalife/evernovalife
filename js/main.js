...
const enlTrack = (function () {
  ...
  if (provider === 'plausible') {
    ...
    /* The per-site "compiled" script Plausible now issues (a pa-<hash>.js URL,
       rather than the old shared script.js) does NOT auto-run on load — its
       last line is `plausible.o && S(plausible.o)`, so it only initializes
       once window.plausible.o already holds a config object. The classic
       script never checks .o and just runs unconditionally, so this stub is
       harmless there — but without it, the compiled variant loads, does
       nothing, and window.plausible is left as this stub forever with no
       error anywhere. Mirrors Plausible's own documented snippet: a second
       stub for .init that stores its argument on .o, called once immediately
       so .o is already set by the time the real script checks for it. */
...
/* ============================================================
   ...
   Seeded SKUs each have their own page (retatrutide-10mg.html and so
   on), generated by tools/build-seo.js and listed in PRODUCT_PAGES.
   That is the address we link and the address we publish: one URL per
   product, with its own title, description and canonical. A product
   added in admin has no generated page, so it keeps the old shared
   URL — which still works, and still resolves to the right SKU.

   Every link built from these carries data-product-id as well, because
   the quick-view handler used to find product links by matching
   "product.html?id=" in the href, and the href is no longer that.
   ============================================================ */
function productSlug(id) {
  const map = (typeof PRODUCT_PAGES !== 'undefined' && PRODUCT_PAGES) || {};
  return map[id] || null;
}

function productHref(product, hash = '') {
  const id = product && product.id !== undefined ? product.id : product;
  const slug = productSlug(id);
  return (slug ? `${slug}.html` : `product.html?id=${encodeURIComponent(id)}`) + hash;
}

function productPageUrl(product) {
  return SITE_ORIGIN + productHref(product);
}
...
function createProductCard(product) {
  ...
  <article class="product-card glass glass-hover">
    <div class="product-media">
      ...
      <a href="${productHref(product)}" data-product-id="${product.id}" aria-label="${escapeHtml(product.name)}">${createVialPhoto(product)}</a>
    </div>
    <div class="product-info">
      <span class="product-cat">${escapeHtml(product.categoryName)}</span>
      <h3 class="product-name"><a href="${productHref(product)}" data-product-id="${product.id}">${escapeHtml(product.name)}</a></h3>
      ...
      <div class="product-actions">
        ${addBtn}
        <a class="btn btn-ghost btn-sm" href="${productHref(product)}" data-product-id="${product.id}">View</a>
      </div>
      <a class="product-coa-link" href="${productHref(product, '#coa')}" data-product-id="${product.id}">${iconCheck()} Certificate of Analysis${
...
function promoHref(promo) {
  ...
  return ids.length === 1 ? productHref(ids[0]) : 'products.html';
}
...
function rerenderProducts() {
  ...
  if (page === '' || page === 'index.html') {
    ...
    // Every generated per-SKU page has its own filename, so the detail view is
    // found by its root element rather than by what the file is called.
...
function renderProductNav(root, product) {
  ...
  const arrow = (p, dir, label) => `
    <a class="pdt-nav pdt-nav-${dir}" href="${productHref(p)}" data-product-id="${p.id}" data-pdt-nav="${dir}"
  ...
  return `
    ...
    <div class="product-detail-info">
      ...
      <div class="detail-cta">
        ...
        ${modal
          ? `<a class="btn btn-ghost btn-lg" href="${productHref(product)}">Full details</a>`
...
function openProductModal(id) {
  ...
  if (!product) { location.href = productHref(id); return false; }
  ...
  try {
    history.pushState({ pdModal: product.id }, '', productHref(product));
...
function pdModalGo(id) {
  ...
  try {
    history.replaceState({ pdModal: product.id }, '', productHref(product));
...
/* Card image, product name, "View", mini-cart and wishlist rows all point at a
   product page — intercepting the link itself covers every one of them,
   including cards rendered later from the API.

   They carry data-product-id, because the href is now a per-SKU page and no
   longer spells out which product it is. The old query URL is still matched so
   a link written before that change, or one to an admin-added product, behaves
   the same. */
function initProductQuickView() {
  /* On a detail page the whole page IS the detail view; related products there
     should navigate rather than stack a modal over an identical layout. Keyed
     on the detail root rather than the filename, since every generated per-SKU
     page has a filename of its own. */
  if (document.getElementById('productDetail')) return;

  document.addEventListener('click', (e) => {
    if (e.defaultPrevented || e.button !== 0) return;
    if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;   // open-in-new-tab
    const link = e.target.closest && e.target.closest('a[data-product-id], a[href*="product.html?id="]');
...
function initProductDetailPage() {
  ...
  if (!root) return;
  /* A generated per-SKU page states its own id; the shared product.html still
     reads it off the query string. */
  const id = window.ENL_PRODUCT_ID || new URLSearchParams(location.search).get('id') || 1;
  const product = getProductById(id);

  /* Landing on the shared URL for a product that has its own page: send the
     visitor (and the crawler) to the address we publish, so the two URLs stop
     competing for the same listing. replace(), not assign(), so Back still
     goes where the visitor came from. Guarded on there being a page to go to,
     which is what makes a loop impossible. */
  if (product && !window.ENL_PRODUCT_ID && productSlug(product.id)) {
    location.replace(productHref(product) + location.hash);
    return;
  }
  ...
  /* The shared product.html ships a generic title and has to be told which SKU
     it is showing. A generated page already has one written for it — replacing
     it here would throw away the longer, indexed version for a shorter one. */
  if (!window.ENL_PRODUCT_ID) document.title = `${product.name} — Ever Nova Life`;
  ...
  return `
    <div class="cart-row glass${warn ? ' has-warn' : ''}" data-id="${item.id}">
      ...
      <div class="cart-row-info">
        ...
        <h4><a href="${productHref(item)}" data-product-id="${item.id}">${name}</a></h4>
...
function renderMiniCart(panel) {
  ...
  const rows = cart.items.map(i => {
    ...
    return `<a class="mini-cart-row" href="${productHref(i)}" data-product-id="${i.id}">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Detail pages are matched by their root element, not their filename — there
  // is one file per SKU now, and product.html is only the fallback among them.
