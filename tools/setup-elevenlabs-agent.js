  ...
  return [
    ...
    },
    {
      ...
      api_schema: {
        ...
        request_headers: {
          ...headers,
          /* An OBJECT, not the string '{{account_token}}'. Header values
             take one of four shapes here — a literal string, a stored
             secret, an env var, or this: a binding to a conversation
             dynamic variable. A template string is none of them, so it is
             sent through verbatim, and the server then rejects the literal
             text '{{account_token}}' as an expired session — a 401 on
             every conversation, with nothing in it that looks like a
             misconfiguration.

             The binding still keeps the token away from the model: it is
             resolved from the conversation's own variables, never
             generated by the LLM, which is why this tool still takes no
             parameters the model can fill in. */
          'x-account-token': { variable_name: 'account_token' }
        },
